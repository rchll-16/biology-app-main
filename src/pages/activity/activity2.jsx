...
const multipleChoiceQuestions = [
  {
    question: 'Which structure controls all the activities of the cell and stores genetic material?',
    options: ['Cytoplasm', 'Golgi body', 'Nucleus', 'Vacuole'],
    correctAnswer: 2, // C. Nucleus
    explanation: 'The nucleus controls all the activities of the cell and stores genetic material.'
  },
  {
    question: 'What is the main function of mitochondria in a cell?',
    options: ['Producing proteins', 'Breaking down waste', 'Generating energy (ATP)', 'Storing pigments'],
    correctAnswer: 2, // C. Generating energy (ATP)
    explanation: 'Mitochondria are responsible for generating energy (ATP) for the cell.'
  },
  {
    question: 'Which organelle is responsible for packaging and transporting proteins and lipids?',
    options: ['Endoplasmic Reticulum', 'Golgi Body', 'Lysosomes', 'Centrosome'],
    correctAnswer: 1, // B. Golgi Body
    explanation: 'The Golgi body is responsible for packaging and transporting proteins and lipids.'
  },
  {
    question: 'What part of the cell is described as a thin, flexible, and semi-permeable membrane?',
    options: ['Cytoskeleton', 'Cell Wall', 'Plasma Membrane', 'Vacuole'],
    correctAnswer: 2, // C. Plasma Membrane
    explanation: 'The plasma membrane is a thin, flexible, and semi-permeable membrane that surrounds the cell.'
  },
];

const trueFalseQuestions = [
  {
    question: 'The rough endoplasmic reticulum is involved in lipid synthesis.',
    correctAnswer: false,
    explanation: 'The smooth endoplasmic reticulum is responsible for lipid synthesis, while the rough ER is involved in protein synthesis.'
  },
  {
    question: 'Lysosomes help clean the cell by breaking down waste materials.',
    correctAnswer: true,
    explanation: 'Lysosomes contain enzymes that break down waste materials and cellular debris.'
  },
  {
    question: 'Vacuoles are only found in animal cells.',
    correctAnswer: false,
    explanation: 'Vacuoles are found in both plant and animal cells, but they are typically larger in plant cells.'
  },
];

const identificationQuestions = [
  {
    question: 'Known as the “powerhouse” of the cell.',
  ...
  {
    question: 'Organizes microtubules and plays a key role in cell division.',
    correctAnswer: 'Centrosome',
    explanation: 'The centrosome organizes microtubules and is crucial for cell division.'
  },
  {
    question: 'Contains chlorophyll and is involved in photosynthesis.',
        ...
        <div className="grid gap-2">
          {['True', 'False'].map((val, i) => (
            <button
              ...
              className={`px-4 py-2 rounded text-white transition ${selected === (val.toLowerCase() === 'true') ? 'bg-blue-700' : 'bg-blue-500 hover:bg-blue-600'}`}
              disabled={answered}
            >
              {val}
            </button>
...
const QuizResults = ({ score, total }) => {
  ...
  const saveQuizResults = async (username, score, total) => {
    try {
      const resultsRef = ref(database, `users/${username}/quizResults/activity2`); // Save results for activity2
      await set(resultsRef, {
        score: score,
        total: total,
        timestamp: new Date().toISOString() // Optional: Add timestamp
      });
      console.log(`Quiz results for activity2 saved to Firebase!`);
    } catch (error) {
      console.error(`Error saving quiz results for activity2:`, error);
  ...
  const handleBackToLesson = async () => {
    ...
    try {
      ...
      await saveLeaderboardData(username, score, 'activity2');

      // Save quiz results
      await saveQuizResults(username, score, total);

      // Mark lesson as completed and unlock next lesson
      const lessonRef = ref(database, `users/${username}/completedLessons/lesson3`); // Changed to lesson3
      await set(lessonRef, {
        score: score,
        total: total,
        completed: true,
      ...
      // Unlock next lesson
      const nextLessonRef = ref(database, `users/${username}/completedLessons/lesson4`); // Changed to lesson4
      ...
      <div className="w-full max-w-2xl p-6 border rounded shadow bg-white">
        <AnimatePresence mode="wait">
          {step === 0 && (
            <InstructionScreen
              key="greeting"
              text="👋 Welcome to the Cell Structure and Function Quiz!"
              duration={3000}
              onDone={() => setStep(1)}
            />
          )}
